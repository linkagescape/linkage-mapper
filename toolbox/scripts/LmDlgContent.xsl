  ...
  if ((window.document.body.clientWidth - 70) > 0)
  {
    <xsl:for-each select="Properties/PropertyGroup">
      <xsl:for-each select="Property">  <xsl:value-of select="CtrlName"/>.width = window.document.body.clientWidth - 30;
        <!-- LM Addition -->
        <xsl:choose>
          <xsl:when test='PropertyName="Param7" or PropertyName="Param8" or PropertyName="Param12" or PropertyName="Param13" or PropertyName="Param17"'>
  ...
  <TABLE BORDER="0" CELLSPACING="0" CELLPADDING="0" WIDTH="100%">
    <TR valign="top">
      ...
      <xsl:choose>
            ...
            <TABLE>
              <TR>
                <xsl:choose>
                  ...
                  <xsl:when test='PropertyName="Param10" or PropertyName="Param14" or PropertyName="Param16"'>
                    <TD class="lmsteppad">
                      <OBJECT width="100%" style="z-index: -1" classid="CLSID:{CtrlCLSID}" id="{CtrlName}" onfocus="ShowHelpTopic('{PropertyName}Topic');" />
                    </TD>
                  </xsl:when>
        ...
        <xsl:otherwise>
          <xsl:choose>
            <xsl:when test='PropertyName="Param7" or PropertyName="Param8" or PropertyName="Param12"
              or PropertyName="Param13" or PropertyName="Param17"'>
              <TD class="lmsteppad">
