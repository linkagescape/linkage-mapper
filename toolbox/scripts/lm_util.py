...
def clean_out_workspace(ws):
    ...
    try:
        delete_data(arcpy.ListDatasets())
...
def make_raster_paths(no_rast, base_dir, sub_dir):
    ...
    Outputs are written to: base\sub for rasters 1-99, base\base1 for rasters
    100-199, etc.
    """
    delete_dir(base_dir)
    try:
        os.makedirs(os.path.join(base_dir, sub_dir))
        for dir_no in range(0, no_rast / 100):
