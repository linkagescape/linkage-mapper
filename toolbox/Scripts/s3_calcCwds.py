    ...
    try:
        ...
        if (BUFFERDIST) is not None:
            GP.addmessage('\nBounding circles plus a buffer of ' +
                          str(float(BUFFERDIST)/1000) + ' km will be used \n'
                          'to limit extent of cost distance calculations.')
        else:
            GP.addmessage('NOT using bounding circles in cost distance '
                          'calculations.')
        ...
        if BUFFERDIST is not None:
            ...
            for x in range(0, numLinks):
                ...
                if linkTable[x,linkTypeCol] == 2:
                    ...
                    cores = zeros((1,3), dtype='int32')
                    cores[0,:] = sort([0, linkTable[x,core1Col],
                                    linkTable[x,core2Col]])
                    corex = cores[0,1]
                    corey = cores[0,2]
                    cores[0,0] = linkId
                    ...
                    if not foundFlag:
                        circlePointData = (lu.get_bounding_circle_data
                                           (extentBoxList, corex, corey,
                                            BUFFERDIST))
                        boundingCirclePointArray = (append
                                                    (boundingCirclePointArray,
                                                    circlePointData, axis=0))
                        # keep track of which cores we draw bounding circles
                        # around
                        circleList = append(circleList, cores, axis=0)
            ...
            GP.addmessage('Successfully created bounding circles around '
                          'potential corridors using \na buffer of ' +
                          str(float(BUFFERDIST)/1000) + ' km.')
            startTime, hours, mins, secs = lu.elapsed_time(startTime)

            GP.addmessage('Reducing global processing area using bounding '
        ...
        for x in range(len(coresToMap)):
            ...
            if len(targetCores)>0:
                ...
                if BUFFERDIST is not None:
                    ...
                    GP.addmessage('Successfully extracted a reduced resistance'
                                  ' raster using')
                ...
                for y in range(len(targetCores)):
                    ...
                    #  cwds in above code
                    if (linkTable[rows[0],linkTypeCol] < 100 and
                        linkTable[rows[0],cwDistCol] != -1):
                        # Flag so that we only evaluate this pair once
                        linkTable[rows,linkTypeCol] = (linkTable
                                                       [rows,linkTypeCol]
                                                       + 100)
                        ...
                        if S3DROPLCCSic:
                            ...
                            GP.SelectLayerByAttribute(FCORES,
                                                      ...
                                                      str(int(sourceCore)))
                            count = 0
                            statement = ('GP.zonalstatisticsastable(FCORES, '
                                         'COREFN, "lcp", ZNSTATS, "DATA", '
                                         '"MINIMUM")')
                            while True:
                                try: exec statement
                                except:
                            ...
                            if coreMin != 'Failed':
                                lu.dashline(1)
                                GP.addmessage("Found an intermediate core in "
                                              "the least-cost path between "
                                              "cores " + str(int(sourceCore)) +
                                              " and " + str(int(targetCore)) +
                ...
                GP.addmessage('Intermediate core checks and LCP shapefiles for'
                              ' core #' + str(sourceCore) + ' took ' +
                              str(processTime) + ' seconds.')
                # -----------------------------------------------------------


                GP.addmessage('Done with all calculations for core #' +
                              str(sourceCore) + '.')
