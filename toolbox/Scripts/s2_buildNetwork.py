    ...
    try:
        ...
        if cwdAdjFile is not None or eucAdjFile is not None:
            ...
            if eucAdjFile is not None:
                adjList = loadtxt(eucAdjFile, dtype = 'int32', comments='#',
                                  delimiter=',') # creates a numpy array
            ...
            if S2ADJMETH_CW:
                ...
                distanceMatrix = multiply(distanceMatrix,cwdAdjMatrix)
                ...
                distanceMatrix = multiply(distanceMatrix,eucAdjMatrix)
                ...
                distanceMatrix = multiply(distanceMatrix, adjMatrix)
        ...
        if dropFlag==True:
            lu.dashline(1)
            GP.addmessage('NOTE: At least one adjacent link was dropped '
                          'because there was no Euclidean ')
            GP.addmessage('distance value in the input distance file from '
                          'Conefor extension.')
            lu.dashline(2)
        ...
        # for adjacent cores from getAdjacencies.py
        linkTable[:,cwDistCol]=-1

        # Update linkTable with new core IDs
        numCores = len(coreList)
        for core in range(numCores):
            if coreList[core,0] != coreList[core,1]:
                linkTable[:,core1Col] = (where(linkTable[:,core1Col] ==
                                         coreList[core,0], coreList[core,1],
                                         linkTable[:,core1Col]))
                linkTable[:,core2Col] = (where(linkTable[:,core2Col] ==
                                         coreList[core,0], coreList[core,1],
                                         linkTable[:,core2Col]))

        # Set linkTypeCol to 2 for all valid corridors (those not yet grouped,
        # i.e. not assigned value of 1)
        ...
        if len(unique(coreList[:,1]))<2:
            GP.addmessage('\n***WARNING: There are less than two core areas. '
                          '\nThis means there is nothing to connect with '
                          'linkages.')
        #----------------------------------------------------------------------
